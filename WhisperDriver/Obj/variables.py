...
class WhisperTradesVariables(object):
    ...
    def __init__(self, endpts):
        self._endpts = endpts
        self.unassociated_variable_numbers = []
        self.variables_list = self.__variable_list(self._endpts)
        self.update_all_variables()

    def __call__(self, variable_number):
        for vari in self.variables_list.all:
            if vari.number == variable_number:
                return vari
        warnings.warn(f"Variable Number: {variable_number} not found!")
        return
